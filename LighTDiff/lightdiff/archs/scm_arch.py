...
class Condition(nn.Module):
    ...
    def forward(self, x):
        ...
        out = torch.mean(conv3_out, dim=[2, 3], keepdim=False) # [B,32]
...
class SpatialControlModule(nn.Module):
    def __init__(self, in_nc=3, out_nc=3, base_nf=64, cond_nf=32, act_type='sigmoid', normal01=False):
        super(SpatialControlModule, self).__init__()
        ...
        self.convt1 = FeatureWiseAffine(base_nf, in_nc)
        self.conv2 = CALayer(base_nf,reduction=4)
        self.convt2 = FeatureWiseAffine(base_nf, base_nf)
        self.conv3 = nn.Conv2d(base_nf, out_nc, 1, 1, bias=True)
        self.convt3 = FeatureWiseAffine(base_nf, base_nf)
