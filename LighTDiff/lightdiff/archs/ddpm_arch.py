...
class GaussianDiffusion(nn.Module):
    ...
    def set_new_noise_schedule(self, schedule_opt, device):
        ...
        assert schedule_opt['n_timestep'] % len(self.structure) == 0
        segment_length = schedule_opt['n_timestep'] // len(self.structure)
        downsampling_schedule = []
        for downsampling_scale in self.structure:
    ...
    def ddim_LighT_sample(
        ...
        assert len(structure) == ddim_timesteps, f'len(structure):{len(structure)} != ddim_timesteps{ddim_timesteps}'
        ...
        init_h = h // structure[-1]
        init_w = w // structure[-1] 

        sample_img = torch.randn((b, c, init_h, init_w), device=device) 
        sample_inter = (1 | (ddim_timesteps//10))  # 1
        ret_img = x_in[:, :3, :, :]
        for i in tqdm(reversed(range(0, ddim_timesteps)), desc='sampling loop time step', total=ddim_timesteps): 
            ...
            if self.controller_module and t > self.color_limit:
                if (not color_gamma) or color_scale > color_gamma:
                    for _ in range(color_times):
                        pred_x0.clamp_(-1., 1.)
                        pred_x0 = self.controller_module(pred_x0, noise_level)
            ...
            if i != 0 and structure[i] != structure[i - 1]: 
                upsample_scale = structure[i] // structure[i - 1]
    ...
    def p_losses_lighT_structure(self, x_HR, x_SR, noise=None, different_t_in_one_batch=False, clip_noise=False, t_range=None, cs_on_shift=False,
        ...
        self.x_recon_cs = self.controller_module(self.x_recon, continuous_sqrt_alpha_cumprod)
    ...
    def forward(self, x_HR, x_SR, train_type='ddpm', *args, **kwargs):
        ...
        elif train_type == 'lighT_structure':
            return self.p_losses_lighT_structure(x_HR, x_SR, *args, **kwargs)
        else:
